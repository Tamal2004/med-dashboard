...
const composeMutations = mutations => {
    const throttleSize = 10;

    mutations.forEach(({ mutation, entity, data }) => {
        const mutationData = [];

        for (let i = 0; i < data.length; i += throttleSize) {
            mutationData.push(data.slice(i, i + throttleSize));
        }

        const mutationStrings = mutationData.map(
            (mutationDatum, mutationIdx) =>
                `${mutation}${mutationIdx}:${mutation}(${entity}: [ ${composeDataString(
                    mutationDatum
                )} ]) { id }`
        );

        const fullMutationString = `mutation loadDB { ${mutationStrings.join(
            ' '
        )} }`;
...
const composeProjects = (projects, clientIds) =>
    projects.reduce(
        (
            ...
            const updatedDate = composeProjectUpdatedAt({
                testingDate,
                ...project
            });

            const dateThreshold =
                new Date().getTime() - 365.25 * 24 * 60 * 60 * 1000 * 3;

            console.log(new Date(updatedDate), updatedDate > dateThreshold);

            if (updatedDate !== 'Invalid' && updatedDate > dateThreshold) {
                ...
                return {
                    ...
                    recentClientIds: [...acm.recentClientIds, clientIds[client]]
                };
            } else {
                return acm;
            }
        },
        { projectIds: {}, projectData: [], recentClientIds: [] }
    );
...
const composeFullQuery = () => {
    ...
    const { projectIds, projectData, recentClientIds } = composeProjects(
        projects,
        clientIds
    );

    const uniqueRecentClients = new Set([...recentClientIds]);

    console.log('recent', uniqueRecentClients);
